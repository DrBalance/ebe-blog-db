...
// ─────────────────────────────────────────────
// POST /analyze-topics — 배치 분석 안내
// 실제 분석은 crawler/analyze_topics.py가 담당
// ─────────────────────────────────────────────
app.post('/analyze-topics', async (req, res) => {
  try {
    const countResult = await pool.query(`
      ...
      WHERE content_ko != '' AND title_ko != ''
        AND NOT EXISTS (SELECT 1 FROM topics t WHERE t.post_id = posts.id)
    `);
    const remaining = parseInt(countResult.rows[0].count);
    const totalResult = await pool.query('SELECT COUNT(*) FROM topics');
    const totalSaved = parseInt(totalResult.rows[0].count);

    res.json({
      message: 'analyze_topics.py를 백그라운드로 실행해주세요.',
      command: 'nohup python -u analyze_topics.py > topics.log 2>&1 &',
      remaining_posts: remaining,
      topics_saved: totalSaved,
    });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: e.message });
...
// ─────────────────────────────────────────────
// POST /analyze-questions — 배치 분석 안내
// 실제 분석은 crawler/analyze_questions.py가 담당
// ─────────────────────────────────────────────
app.post('/analyze-questions', async (req, res) => {
  try {
    ...
    res.json({
      message: 'analyze_questions.py를 백그라운드로 실행해주세요.',
      command: 'nohup python -u analyze_questions.py > questions.log 2>&1 &',
